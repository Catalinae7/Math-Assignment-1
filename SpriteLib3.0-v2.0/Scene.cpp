...
void Scene::AdjustScrollOffset()
{
	...
	float playerHalfSizex = ECS::GetComponent<Sprite>(MainEntities::MainPlayer()).GetWidth() / 0.3f;
	float playerHalfSizey = ECS::GetComponent<Sprite>(MainEntities::MainPlayer()).GetHeight() / 0.3f;

	ECS::GetComponent<HorizontalScroll>(MainEntities::MainCamera()).SetOffset((maxSizeX * BackEnd::GetAspectRatio()) - playerHalfSizex);
	ECS::GetComponent<VerticalScroll>(MainEntities::MainCamera()).SetOffset(maxSizeY - playerHalfSizey);
}
